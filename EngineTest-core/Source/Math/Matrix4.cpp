...
namespace EngineTest { namespace Math {
	...
	Matrix4 Matrix4::Orthographic(double left, double right, double top, double bottom, double near, double far)
	{
		...
		result[0] = (float)(2.0 / (right - left));
		result[5] = (float)(2.0 / (top - bottom));
		result[10] = (float)(2.0 / (near - far));
		result[12] = (float)((left + right) / (left - right));
		result[13] = (float)((bottom + top) / (bottom - top));
		result[14] = (float)((far + near) / (far - near));
	...
	Matrix4 Matrix4::Translate(const Vector3& translate)
	{
		...
		result[12] = translate._x;
		result[13] = translate._y;
		result[14] = translate._z;
	...
	Matrix4 Matrix4::Rotate(float radians, const Vector3& unitVec)
	{
		...
		result[0] = (float)(x * x * omc + c);
		result[1] = (float)(x * y * omc - z * s);
		result[2] = (float)(x * z * omc + y * s);

		result[4] = (float)(y * x * omc + z * s);
		result[5] = (float)(y * y * omc + c);
		result[6] = (float)(y * z * omc - x * s);

		result[8] = (float)(z * x * omc - y * s);
		result[9] = (float)(z * y * omc + x * s);
		result[10] = (float)(z * z * omc + c);
	...
	Matrix4 Matrix4::Rotate(const Quaternion& quat)
	{
		...
		result[0] = (1.0f - qyqy2) - qzqz2;
		result[4] = qxqy2 - qzqw2;
		result[8] = qxqz2 + qyqw2;

		result[1] = qxqy2 + qzqw2;
		result[5] = (1.0f - qxqx2) - qzqz2;
		result[9] = qyqz2 - qxqw2;

		result[2] = qxqz2 - qyqw2;
		result[6] = qyqz2 + qxqw2;
		result[10] = (1.0f - qxqx2) - qyqy2;

		return result;
	}
	Matrix4 Matrix4::Simple2DTransform(const Vector3& position, const Vector2& scale, float radians)
	{
		Matrix4 result;
		...
		result[5] = scale._y * c;
		result[10] = 1.0f;
		result[12] = position._x;
		result[13] = position._y;
		result[14] = position._z;
		result[15] = 1.0f;

		return result;
	}
	void Matrix4::Simple2DTransform(Matrix4& matrix, const Vector3& position, const Vector2& scale, float radians)
	{
		float c = cos(radians);
		float s = sin(radians);
		...
		matrix[5] = scale._y * c;
		matrix[10] = 1.0f;
		matrix[12] = position._x;
		matrix[13] = position._y;
		matrix[14] = position._z;
		matrix[15] = 1.0f;
	}
